...
<script type="text/javascript">
 $(function() {
	...
	$(".weipaichu").click(function(){
		window.parent.document.getElementById("rescuecar").value = $(this).find("#rescuecar").val();
		window.parent.document.getElementById("carId").value = $(this).find("#carId").val();
		window.parent.document.getElementById("driver").value = $(this).find("#driver").val();
		window.parent.document.getElementById("driverphone").value = $(this).find("#driverphone").val();
		window.parent.document.getElementById("driverId").value = $(this).find("#driverId").val();
		...
		<ul class="ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all" role="tablist"></ul>
						...
						<c:forEach items="${cars}" var="car">
							<div class="mainbox" style="height:1000px;">
								<div class="infocard-dialog">
									...
									<input type="hidden" name="rescuecar" id="rescuecar" value="${car.carnum}"/>
									<input type="hidden" name="driver" id="driver" value="${car.driver.name}"/>
									<input type="hidden" name="driverId" id="driverId" value="${car.driver.id}"/>
									<input type="hidden" name="driverphone" id="driverphone" value="${car.driver.phone}"/>
								</div> 
							</div>
